...
into_path = r'D:\paddleocr'  # 微信image文件路径
out_path = r"D:\paddleocr/jpg"
...
def dat2img_main(into_file_path):
    path_arr = into_file_path.split('\\')
    dat_file = path_arr[len(path_arr)-1]
    dat_file_name = dat_file[:-4]  # 截取字符串 去掉.dat
    out_path = '\\'.join(path_arr[:-1])
    return imageDecode2(into_file_path, dat_file_name, out_path)  # 转码函数
...
def imageDecode2(temp_path, dat_file_name, out_path):
    dat_read = open(temp_path, "rb")  # 读取.bat 文件
    xo, j = Format(temp_path)  # 判断图片格式 并计算返回异或值 函数
...
#     out = dat2img_main(r'D:\paddleocr\wuye.dat')
#     print(out)
